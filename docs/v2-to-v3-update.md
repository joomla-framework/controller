...
`ControllerInterface::execute()` and every method on `AbstractController` — `getApplication()`,
`setApplication()`, `getInput()`, `setInput()` — have the same signature in 3.0.0 as in 2.0.0.
...
The optional packages in `suggest` moved to their 3.x releases: `joomla/application` and
`joomla/input`.
